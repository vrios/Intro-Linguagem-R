...
# oque as funções abaixo fazem? O que elas retornam?
is.vector()
is.numeric()
is.character()
is.matrix(peso)
...
#encontre os valores máximo e mínimo do seguinte conjunto de dados (usando funções do R, não manualmente). Armazene esses valores em objeto chamados maior e menor
...
#Usando o conjunto de dados caixeta.csv, disponivel em http://ecologia.ib.usp.br/bie5782/doku.php?id=dados:dados-caixeta, calcule quantas espécies estão presentes na coluna especie
...
#podemos criar uma coluna a mais no nosso dataframe apenas dizendo ao R o que colocar lá. Por exemplo, criamos uma coluna com o nome do coletor do dado
 #o que as funções rep(), C() e length() estão fazendo neste comando?
...
#A função table() também pode contar por espécie em cada local. Como?
table(caixeta$especie, caixeta$h)
...
#crie um ciclo (for) que mostre todos os inteiros de 1 até 10, um por um

#crie um ciclo (for) que some todos os inteiros de 1 até 10, guardando esses valores num objeto soma (dica crie o objeto soma com valor zero antes do código do ciclo)
