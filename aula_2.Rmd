...
Precisamos saber a posição dos elementos com erro:
...
i==0.15
```
Resumidamente, se você quiser comparar com precisão a igualdade de operações com decimais, use a função `all.equal()`
```{r}
i = 0.1
i = i + 0.05
all.equal (i,0.15)
...
```
***
Por exemplo, se quisermos todos os dragões que pesam entre 100 e 150 dezenas de kilos
```{r}
...
***
#if()
`if()` é uma função que permite executar um recho de código se, e somente se, uma dada condição for verdadeira
